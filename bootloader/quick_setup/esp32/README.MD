...
> scaffolding that belongs to the UART project. They live in the UART source folder.
...
└── esp32/
    ├── src/
    │   ├── boot.S        ← entry point and stack setup
    │   ├── main.c        ← your UART driver goes here
    │   └── uart.c        ← driver implementation (created in uart phase)
    ├── linker.ld         ← memory layout
    ├── Makefile          ← run all make commands from uart/esp32/
    └── README.md
```

> Always run `make` from `uart/esp32/` — that is where `Makefile` and `linker.ld` live.
> Running it from any other directory will fail because the Makefile references
> `linker.ld` and `src/` by relative path.
...
> Run all `make` commands from `uart/esp32/` — where this Makefile lives.
...
```bash
cd uart/esp32/     # always run make from here
make               # build
make run           # run in QEMU
make debug         # run with GDB server on port 1235
```
...
- [`../../uart/esp32/README.md`](../../uart/esp32/README.md)
